...
void OLED_printKey(byte buf[8], byte msgType = 0) {
  ...
  String mode;
  switch (copierMode) {
    case 1: mode = "*READING*"; break;
    case 2: mode = "**WRITING**";  break;
    case 0:
    case 3: mode = "***LIVE MODE***";  break;
  }


  myOLED.clrScr();
  myOLED.invertText(true);
  myOLED.print(mode, CENTER, 0);
  ...
  myOLED.print(st, CENTER, 36);
  //  myOLED.print(st2, 0, 18);
  st = "Type ";
  switch (keyType) {
...
void setup() {
  ...
  myOLED.begin(SSD1306_128X64); //инициализируем дисплей
  ...
  enc1.setType(TYPE2);
  enc1.setDirection(NORM);         // NORM / REVERSE
...
void clearLed() {
  ...
  switch (copierMode)
  {
    case md_read: digitalWrite(G_Led, HIGH); digitalWrite(Luse_Led, HIGH); Serial.println("ReadMode"); break;
    case md_write: digitalWrite(R_Led, HIGH); digitalWrite(Luse_Led, HIGH); Serial.println("WriteMode"); break;
    case md_blueMode: digitalWrite(B_Led, HIGH); digitalWrite(Luse_Led, LOW); Serial.println("BlueMode"); break;
  }
}
